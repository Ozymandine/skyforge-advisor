...
export async function resolveUuid(username: string): Promise<{ uuid: string; name: string } | null> {
  try {
    ...
    console.error("Failed to resolve UUID:", e);
  }
  return null;
}
...
export async function getBazaar(): Promise<{ lastUpdated: number; products: BazaarProduct[] }> {
  try {
    ...
    if (!data || !data.products) {
      return { lastUpdated: Date.now(), products: [] };
    }
    ...
    return { lastUpdated: data.lastUpdated || Date.now(), products };
  } catch (err) {
...
export async function getAuctions(pages = 6): Promise<{
  ...
  try {
    ...
    if (!first || !Array.isArray(first.auctions)) return fallback;

    const wanted = Math.min(pages, first.totalPages || 1);
    ...
    const all = [
      first,
      ...rest.filter((v): v is AuctionsResponse => v !== null && Array.isArray(v.auctions)),
    ];
...
function getCollectionCategory(itemId: string): string {
  ...
  if (
    id.includes("LOG") ||
    id.includes("WOOD") ||
    ["JUNGLE", "ACACIA", "DARK_OAK", "BIRCH", "SPRUCE", "OAK"].some((k) => id.includes(k))
  ) {
    return "Foraging";
  }
  ...
  // Rift / Special
  if (id.includes("RIFT")) return "Rift";
...
export async function getPlayerData(
  ...
  try {
    ...
    const chosen =
      list.find((p) => p.profile_id === profileId) ?? list.find((p) => p.selected) ?? list[0]!;

...
const matchKeys = (stem) => {
  ...
  if (words.length > 1 && words.at(-1)?.endsWith("s"))
    keys.add([...words.slice(0, -1), words.at(-1).slice(0, -1)].join("_"));
  if (words.length > 1 && !words.at(-1)?.endsWith("s"))
    keys.add([...words.slice(0, -1), `${words.at(-1)}s`].join("_"));
  if (words.length > 1 && !words[0].endsWith("s"))
    keys.add([`${words[0]}s`, ...words.slice(1)].join("_"));
...
const buildRegistry = async (directory, prefix) => {
  const files = (await readdir(directory)).filter(
    (file) => /\.(png|jpg|jpeg|webp)$/i.test(file) && !/_model\.(png|jpg|jpeg|webp)$/i.test(file),
  );
  ...
  for (const path of Object.values(items)) {
    ...
    for (const key of matchKeys(stem)) {
      ...
      const score = (candidate) => [
        candidate.includes("/items/") ? 0 : 1,
        candidate.includes(`${key}.`) ? 0 : 1,
        candidate.length,
        candidate,
      ];
...
try {
  const aliases = JSON.parse(
    await readFile(join(root, ".output", "wiki-icon-aliases.json"), "utf8"),
  );
  for (const [key, file] of Object.entries(aliases)) {
    const path = `/items/${file}`;
    if (Object.values(items).includes(path)) matches[normalizeKey(key)] = path;
  }
} catch {
  // Alias reports are generated by the wiki sync and are optional for clean checkouts.
}
await writeFile(
  join(root, "src", "lib", "items", "generated-items.json"),
  `${JSON.stringify(items, null, 2)}\n`,
  "utf8",
);
await writeFile(
  join(root, "src", "lib", "items", "generated-vanilla.json"),
  `${JSON.stringify(vanilla, null, 2)}\n`,
  "utf8",
);
await writeFile(
  join(root, "src", "lib", "items", "generated-item-matches.json"),
  `${JSON.stringify(matches, null, 2)}\n`,
  "utf8",
);
console.log(
  `Generated ${Object.keys(items).length} item, ${Object.keys(vanilla).length} vanilla, and ${Object.keys(matches).length} item match entries`,
);

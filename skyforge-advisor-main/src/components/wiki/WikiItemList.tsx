...
export function WikiItemList({ items, selectedId, onSelect }: WikiItemListProps) {
  return (
      ...
      {items.map((item) => (
        <button
          ...
          className={cn(
            ...
            selectedId === item.id && "bg-primary/10 ring-2 ring-primary/40",
          )}
        >
          <WikiItemIcon
            id={item.id}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1.5 flex items-center gap-2">
              ...
              <span className="truncate text-[10px] text-muted-foreground">{item.category}</span>
            </div>
          </div>
        </button>
      ))}

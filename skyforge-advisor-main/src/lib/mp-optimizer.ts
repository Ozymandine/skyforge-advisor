...
export const COMMON_UNOWNED_ACCESSORIES: AccessoryPurchaseRecommendation[] = [
  ...
  },
  {
    ...
    rarity: "MYTHIC",
    mpValue: 22,
    costCoins: 380_000_000,
    costPerMp: 17_272_727,
    upgradeType: "ah_buy",
  },
];

export function getTopMpUpgrades(
  ownedIds: Set<string> = new Set(),
): AccessoryPurchaseRecommendation[] {
  return COMMON_UNOWNED_ACCESSORIES.filter((a) => !ownedIds.has(a.id)).sort(
    (a, b) => a.costPerMp - b.costPerMp,
  );
}
  ...
  return [
    {
      rarity: "LEGENDARY",
      mpGained: 6,
      costPerMpGained: Math.round(recombPrice / 6),
      priorityRating: "HIGHEST",
    },
    {
      rarity: "EPIC",
      mpGained: 4,
      costPerMpGained: Math.round(recombPrice / 4),
      priorityRating: "HIGH",
    },
    {
      rarity: "RARE",
    ...
    },
    {
      rarity: "UNCOMMON",
      mpGained: 3,
      costPerMpGained: Math.round(recombPrice / 3),
      priorityRating: "MEDIUM",
    },
    {
      rarity: "COMMON",
      mpGained: 2,
      costPerMpGained: Math.round(recombPrice / 2),
      priorityRating: "LOW",
    },
  ];
}
